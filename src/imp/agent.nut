...
// Used to send data to vtiot AWS account
#require "AWSRequestV4.class.nut:1.0.2"
#require "AWSSQS.agent.lib.nut:1.0.0"
const AWS_SQS_ACCESS_KEY_ID     = "replaceme";
const AWS_SQS_SECRET_ACCESS_KEY = "replaceme";
const AWS_SQS_URL               = "replaceme";
const AWS_SQS_REGION            = "us-east-1";

// Instantiate the class
sqs <- AWSSQS(AWS_SQS_REGION, AWS_SQS_ACCESS_KEY_ID, AWS_SQS_SECRET_ACCESS_KEY);

local STATION_ID = "replaceme";

local STATION_PW = "replaceme"; //Note that you must only use alphanumerics in your password. Http post won't work otherwise.

local LOCAL_ALTITUDE_METERS = 85; 

local midnightReset = true; //Keeps track of a once per day cumulative rain reset

local local_hour_offset = 5; 
...
device.on("postToInternet", function(dataString) {
    ...
    local strMainSite = "https://weatherstation.wunderground.com/weatherstation/updateweatherstation.php";
    ...
    //bigString += "&" + weather;
    //bigString += "&" + clouds;
    bigString += "&" + "softwaretype=WeatherImp"; //Cause we can
    ...
    local localMeasurementTime = "measurement time: " + calcLocalTime();
    server.log(localMeasurementTime);
...
function calcLocalTime()
{
    ...
    local h = format("%02u", hour);
    local m = format("%02u", currentTime.min);
    local s = format("%02u", currentTime.sec);

    currentTime = DoW + ", " + h + ":" + m + ":" + s + " local";
    return(currentTime);
}
...
function sendToAWS(dataString) {
    ...
    sqs.action(AWSSQS_ACTION_SEND_MESSAGE, sendParams, function(response) {
        //local json = http.jsonencode(response);
        server.log("SQS Response Status: " + response.statuscode);
    });
}